...
class CanvasSection(models.Model):
    RE_SECTION_SIS_ID = re.compile(
        ...
        r"[\w& ]+-"                                 # curriculum
        r"\d{3}-"                                   # course number
        r"[A-Z][A-Z0-9]?(-[A-F0-9]{32})?-{0,2}$",   # section id|regid
        re.VERBOSE)
    ...
    def is_academic_sis_id(self):
        return (self.sis_section_id is not None and
                len(self.sis_section_id) and
                self.RE_SECTION_SIS_ID.match(self.sis_section_id) is not None)
