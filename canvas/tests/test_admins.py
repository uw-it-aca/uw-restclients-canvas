...
from canvas.admins import Admins
import mock
...
class CanvasTestAdmins(TestCase):
    ...
    def test_create_admin(self, mock_post):
        ...
        canvas.create_admin_by_sis_id('uwcourse:seattle:nursing:nurs', 1111, 'AccountAdmin')
        mock_post.assert_called_with(
            '/api/v1/accounts/sis_account_id%3Auwcourse%3Aseattle%3Anursing%3Anurs/admins',
            {'role': 'AccountAdmin', 'send_confirmation': False, 'user_id': '1111'})

    @mock.patch.object(Admins, '_delete_resource')
    def test_delete_admin(self, mock_delete):
        canvas = Admins()

        canvas.delete_admin_by_sis_id('uwcourse:seattle:nursing:nurs', 1111, 'AccountAdmin')
        mock_delete.assert_called_with('/api/v1/accounts/sis_account_id%3Auwcourse%3Aseattle%3Anursing%3Anurs/admins/1111?role=AccountAdmin')
